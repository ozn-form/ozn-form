...
本プロジェクトでは、実装に関する技術的な考慮事項や設計判断を文書化しています。

- [メールアドレス入力フィールドのtype属性とドメインサジェスト機能の両立について](docs/EMAIL_INPUT_TYPE_CONSIDERATIONS.md)
  - `type="email"` とカスタムドメインサジェスト機能の競合問題と解決方法について解説
  - フォーム設置者は、カスタムドメインサジェストまたはブラウザオートコンプリートのどちらを使用するか、HTMLの記述で選択できます
- [マニュアルに記載していない検証ルールについて](docs/UNDOCUMENTED_VALIDATION_RULES.md)
  - `"validates"` に指定可能だが、一般ユーザー向けマニュアルには意図的に非公開としている検証ルールの一覧
  - 設定ファイル上は記述できても、パース仕様の制約により実質的に機能しない検証ルールとその理由

## 変更履歴

### [v2.9.3] - 2026-05-26

- Fixed: DatePicker選択直後の誤バリデーションを防ぐため、onClose後に対象フィールドを再検証

### [v2.9.2] - 2026-03-12

- Fixed: 電話番号バリデーションを改善（日本国内向け桁数チェック：国際電話形式は未対応）

### [v2.9.1] - 2025-10-05

- Fixed: サンプルフォーム samples/normal/ のアクセシビリティ改善
- Added: ozn-form/css/ozn-form.min.css にアクセシビリティ向上のためのスタイルを追加（後方互換考慮）
- Added: マニュアル内、カスタムドメインサジェストまたはブラウザオートコンプリートをHTMLの記述で選べる旨追記
