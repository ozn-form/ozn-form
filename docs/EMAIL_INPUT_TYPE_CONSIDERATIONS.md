...
本実装では、フォーム設置者がHTMLの記述を変更することで、カスタムドメインサジェストとブラウザのオートコンプリートのどちらを使用するか選択できます。

### パターン1: カスタムドメインサジェストを使用（デフォルト・推奨）

```html
<input data-domain-suggest="true" type="text" inputmode="email" 
       name="email" class="ozn-input" 
       placeholder="例）yamada@example.com">
```

**特徴:**
- `data-domain-suggest="true"` を指定すると、JavaScriptが自動的に `autocomplete="off"` を設定
- カスタムドメインサジェスト（gmail.com、yahoo.co.jp等）が動作
- ブラウザのオートコンプリートは抑制される
- モバイルでメール用キーボードが表示される（`inputmode="email"`）

**推奨ケース:**
- 不特定多数のユーザーが利用するB2Cサイト
- 新規ユーザーが多いフォーム
- 一般的なメールドメインの入力を簡素化したい場合

### パターン2: ブラウザのオートコンプリートを使用

```html
<input type="email" name="email" class="ozn-input" 
       placeholder="例）yamada@example.com">
```

**特徴:**
- `data-domain-suggest` 属性を削除
- `type="email"` を使用（セマンティックHTML）
- ブラウザが過去の入力履歴から自動補完候補を表示
- カスタムサジェストは無効化される

**推奨ケース:**
- 社内システムなど、同じユーザーが繰り返し使用するフォーム
- ユーザーが複数のメールアドレスを使い分ける場合
- アクセシビリティをより重視する場合

### 選択の判断基準

| 観点 | カスタムサジェスト | ブラウザオートコンプリート |
|------|-----------------|----------------------|
| 対象ユーザー | 不特定多数・新規ユーザー | 既存ユーザー・繰り返し利用 |
| 入力支援 | 一般的なドメイン候補 | 個人の入力履歴 |
| プライバシー | ブラウザに履歴を残さない | ブラウザに履歴を保存 |
| モバイルUX | メール用キーボード表示 | メール用キーボード表示 |
| カスタマイズ | 候補リストを自由に設定可能 | ブラウザ依存 |
| アクセシビリティ | やや劣る（type="text"） | 優れる（type="email"） |

この柔軟性により、フォームの目的や対象ユーザーに応じて、最適な入力支援方法を選択できます。
...
1. **デフォルトはカスタムサジェスト** (`type="text"` + `inputmode="email"` + `data-domain-suggest="true"`)
2. **利用者に選択肢を提供**: HTML記述を変更することで、ブラウザオートコンプリートへの切り替えが可能
3. **z-indexの調整**: 万が一ブラウザのオートコンプリートが表示された場合でも、カスタムサジェストが見えるように z-index を高めに設定（9999）
4. **ドキュメント化**: 本ドキュメントで両方の選択肢と判断基準を明示

この問題は、WebフォームのアクセシビリティとカスタムUI機能のトレードオフという、より広範な課題の一例です。プロジェクトの目的（EFO機能の提供）を優先しつつ、可能な範囲でアクセシビリティを向上させ、かつ利用者に選択肢を提供するアプローチが現実的です。
